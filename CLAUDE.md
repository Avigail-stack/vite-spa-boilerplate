...
Note: All commands must be run from the `vite-spa-boilerplate` directory.

## Architecture

### Entry Points
- **index.html**: Root HTML file that mounts the app to `#app` div and loads `/src/main.js`
- **src/main.js**: Application entry point that renders the entire page via innerHTML and sets up interactive features
...
- **Color Palette**: Defined in CSS custom properties (`--color-primary`, `--color-accent`, etc.) in src/style.css:8-17
...
1. **Smooth Scroll Navigation**: Links with `href="#"` scroll smoothly to anchor targets (src/main.js:240-248)
2. **Scroll Animations**: Intersection Observer triggers `.visible` class on feature sections when they enter viewport (src/main.js:251-269)
3. **Navbar Scroll Effect**: Navbar gains `scrolled` class after 50px scroll for enhanced backdrop blur (src/main.js:272-285)
4. **CSS Animations**:
   - Brain wave visualization using animated lines (src/style.css:232-251)
   - Floating device mockup (src/style.css:211-214)
   - Pulsing AI chip visual (src/style.css:332-390)
   - Fade-in animations with staggered delays (src/style.css:709-739)

### Responsive Design

- Breakpoints at 768px and 480px (src/style.css:742-808)
...
### Adding New Sections
When adding new page sections to src/main.js:

1. Add HTML markup within the template literal (after line 237)
2. Add corresponding styles to src/style.css
3. If the section needs scroll animations, add it to the observer (src/main.js:266-268)
